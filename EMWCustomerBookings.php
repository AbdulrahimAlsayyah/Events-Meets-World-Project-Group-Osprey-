...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cancelEvent'])) {
    ...
    if (!$eventID) {
        ...
        $check = $conn->prepare("
            ...
            AND Payment.CustomerFK = ?
        ");

        $check->bind_param("ii", $eventID, $customerID);
        $check->execute();
        ...
        if (!$selectedEvent) {
            ...
            try {
                ...
                $cancel->bind_param("i", $eventID);
                $cancel->execute();

                $conn->commit();

                $message = "Event Cancelled, You will be refunded.";
    ...
    <main class="main">
        ...
        <div class="events-box">
            ...
            <form method="POST" id="cancelEventForm">
                ...
                <p class="scroll-note">scroll down to see more events</p>

                <button type="button" class="black-btn" onclick="openCancelModal()">
                    Cancel Event
                </button>

                <span class="cancel-help">Select an event to cancel</span>
            ...
            <div class="event-search">
                <input
                    type="text"
                    id="eventSearch"
                    placeholder="Search for an event based on event type, date, status or vendor"
                >
...
function selectEvent(card) {
    if (card.classList.contains("disabled")) {
        alert("Only scheduled events can be cancelled.");
        return;
    }
